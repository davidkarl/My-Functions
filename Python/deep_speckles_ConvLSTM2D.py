...
def vision_block(input_layer, number_of_filters_vec, kernel_size_vec, \
...
#################################################################################################################
##Build Convolutional model which acts on each image seperately:
##ANOTHER, MORE ELEGANT, WAY OF DOING THIS WOULD BE TO USE TimeDistriuted layer!@#@!$@#$
#
...
#        
#        conv_model_current_image = vision_block(conv_model_current_image, 
#                                                 number_of_filters, 
#                                                 kernel_size,
...
#conv_models_output_concatenated = keras.backend.tf.variable();
#for i in arange(0,length(conv_models_output_list),1):
#    conv_models_output_concatenated = concatenate([conv_models_output_concatenated,conv_models_output_list[i]]);
#conv_models_output_concatenated = Model(inputs=[image_input_layers[0],image_input_layers[1]], \
#                                          outputs=[conv_models_output_concatenated]);
#################################################################################################################################################################################################################
...
for layer_counter in arange(0,number_of_convolutional_model_layers,1):
    ...
    conv_model_single_image = vision_block(conv_model_single_image, 
