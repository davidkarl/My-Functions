function [orthonormal_filter] = wavelet_get_orthonormal_QMF_filter(wavelet_type_string,p_parameter)
% MakeONFilter -- Generate Orthonormal QMF (Quadrature mirror filter) Filter for Wavelet Transform
%  Usage
...
if strcmp(wavelet_type_string,'Haar'),
    orthonormal_filter = [1 1] ./ sqrt(2);
end

if strcmp(wavelet_type_string,'Beylkin'),
    orthonormal_filter = [	.099305765374	.424215360813	.699825214057	...
...
if strcmp(wavelet_type_string,'Coiflet'),
    if p_parameter==1,
        orthonormal_filter = [	.038580777748	-.126969125396	-.077161555496	...
            .607491641386	.745687558934	.226584265197	];
    end
    if p_parameter==2,
        orthonormal_filter = [	.016387336463	-.041464936782	-.067372554722	...
            .386110066823	.812723635450	.417005184424	...
            -.076488599078	-.059434418646	.023680171947	...
            .005611434819	-.001823208871	-.000720549445	];
    end
    if p_parameter==3,
        orthonormal_filter = [	-.003793512864	.007782596426	.023452696142	...
    ...
    if p_parameter==4,
        orthonormal_filter = [	.000892313668	-.001629492013	-.007346166328	...
    ...
    if p_parameter==5,
        orthonormal_filter = [	-.000212080863	.000358589677	.002178236305	...
...
if strcmp(wavelet_type_string,'Daubechies'),
    if p_parameter==4,
        orthonormal_filter = [	.482962913145	.836516303738	...
            .224143868042	-.129409522551	];
    end
    if p_parameter==6,
        orthonormal_filter = [	.332670552950	.806891509311	...
            .459877502118	-.135011020010	...
            -.085441273882	.035226291882	];
    end
    if p_parameter==8,
        orthonormal_filter = [ 	.230377813309	.714846570553	...
            .630880767930	-.027983769417	...
            -.187034811719	.030841381836	...
            .032883011667	-.010597401785	];
    end
    if p_parameter==10,
        orthonormal_filter = [	.160102397974	.603829269797	.724308528438	...
            .138428145901	-.242294887066	-.032244869585	...
            .077571493840	-.006241490213	-.012580751999	...
            .003335725285									];
    end
    if p_parameter==12,
        orthonormal_filter = [	.111540743350	.494623890398	.751133908021	...
            .315250351709	-.226264693965	-.129766867567	...
            .097501605587	.027522865530	-.031582039317	...
            .000553842201	.004777257511	-.001077301085	];
    end
    if p_parameter==14,
        orthonormal_filter = [	.077852054085	.396539319482	.729132090846	...
    ...
    if p_parameter==16,
        orthonormal_filter = [	.054415842243	.312871590914	.675630736297	...
    ...
    if p_parameter==18,
        orthonormal_filter = [	.038077947364	.243834674613	.604823123690	...
    ...
    if p_parameter==20,
        orthonormal_filter = [	.026670057901	.188176800078	.527201188932	...
...
if strcmp(wavelet_type_string,'Symmlet'),
    if p_parameter==4,
        orthonormal_filter = [	-.107148901418	-.041910965125	.703739068656	...
            1.136658243408	.421234534204	-.140317624179	...
            -.017824701442	.045570345896					];
    end
    if p_parameter==5,
        orthonormal_filter = [	.038654795955	.041746864422	-.055344186117	...
            .281990696854	1.023052966894	.896581648380	...
            .023478923136	-.247951362613	-.029842499869	...
            .027632152958									];
    end
    if p_parameter==6,
        orthonormal_filter = [	.021784700327	.004936612372	-.166863215412	...
            -.068323121587	.694457972958	1.113892783926	...
            .477904371333	-.102724969862	-.029783751299	...
            .063250562660	.002499922093	-.011031867509	];
    end
    if p_parameter==7,
        orthonormal_filter = [	.003792658534	-.001481225915	-.017870431651	...
    ...
    if p_parameter==8,
        orthonormal_filter = [	.002672793393	-.000428394300	-.021145686528	...
    ...
    if p_parameter==9,
        orthonormal_filter = [	.001512487309	-.000669141509	-.014515578553	...
    ...
    if p_parameter==10,
        orthonormal_filter = [	.001089170447	.000135245020	-.012220642630	...
...
if strcmp(wavelet_type_string,'Vaidyanathan'),
    orthonormal_filter = [	-.000062906118	.000343631905	-.000453956620	...
...
if strcmp(wavelet_type_string,'Battle'),
    if p_parameter == 1,
    ...
    l = length(g);
    orthonormal_filter = zeros(1,2*l-1);
    orthonormal_filter(l:2*l-1) = g;
    orthonormal_filter(1:l-1) = reverse_vec(g(2:l));
end

orthonormal_filter = orthonormal_filter ./ norm(orthonormal_filter);