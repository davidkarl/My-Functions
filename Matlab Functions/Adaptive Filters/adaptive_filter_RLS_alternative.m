...
%   RLS.m
%       Implements the RLS alternative algorithm for COMPLEX valued data.
...
for iteration_counter = 1:N_number_of_iterations
    ...
    %calculate current psi factor:
    psi_auxiliary = Sd_inverse_auto_correlation * current_signal;
    
    %update Sd (inverse autocorrelation matrix) using inversion lemma:
    Sd_inverse_auto_correlation = 1/lambda_smoothing_factor *(Sd_inverse_auto_correlation - (psi_auxiliary*psi_auxiliary'))...
                    / (lambda_smoothing_factor + psi_auxiliary'*current_signal);
    ...
    next_coefficients = current_coefficients + conj(current_filter_error)*Sd_inverse_auto_correlation*current_signal;
