...
for iteration_counter = 1:N_number_of_iterations,
    ...
    %Calculate iterative updating for the inverse_Rx matrix using the inverse matrix lemma:
    inverse_Rx_auxiliary_denominator = ((1-Rx_estimate_smoothing_factor)/Rx_estimate_smoothing_factor) + current_signal'*inverse_Rx_estimate*current_signal;
    inverse_Rx_auxiliary_nominator = inverse_Rx_estimate*(current_signal*current_signal')*inverse_Rx_estimate;
    inverse_Rx_estimate = (1/(1-Rx_estimate_smoothing_factor)) * (inverse_Rx_estimate - inverse_Rx_auxiliary_nominator/inverse_Rx_auxiliary_denominator);
   
    
    %calculate next coefficients vector using LMS-newton rule:
    next_coefficients = current_coefficients + ...
                                    (mu_convergence_factor * conj(current_filter_error) * inverse_Rx_estimate * current_signal);
