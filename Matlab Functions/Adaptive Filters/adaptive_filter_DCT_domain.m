...
function    [filter_output_over_time,filter_error_over_time,filter_coefficients_over_time_in_T_domain,coefficient_vector] = ...
    adaptive_filter_DCT_domain(desired_signal,input_signal,filter_order,initial_coefficients,mu_convergence_factor,gamma_normalization_factor,alpha_smoothing_factor,initial_power)
       
%initialize parameters:
number_of_coefficients = filter_order+1;
N_number_of_iterations = length(desired_signal);
T = dctmtx(number_of_coefficients);
...
filter_coefficients_over_time_in_T_domain(:,1) = initial_coefficients;

%initialize auxiliary next coefficients vector and signal power:
next_coefficients_in_T_domain = T*initial_coefficients;
...
for iteration_counter = 1:N_number_of_iterations,
    ...
    current_input_signal_T_transformed = T*current_input_signal;
    ...
    filter_coefficients_over_time_in_T_domain(:,iteration_counter+1) = next_coefficients;
    
end

coefficient_vector = T'*filter_coefficients_over_time_in_T_domain;
