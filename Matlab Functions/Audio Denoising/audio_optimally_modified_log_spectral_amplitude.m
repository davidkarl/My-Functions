function [y,out]=audio_optimally_modified_log_spectral_amplitude(input_file_name,output_file_name)
...
input_file_name = 'Sound0001';

nin=nargin; 
if nin<2
    output_=[input_file_name,'_omlsa'];
end

% 1) Parameters of Short Time Fourier Analysis:
Fs_reference=16e3;
samples_per_frame_reference=512;
overlapping_samples_per_frame_reference=0.75*samples_per_frame_reference;

% 2) Parameters of Noise Spectrum Estimate
frequency_smoothing_window_half_size=1;
...
Vwin=15;
delta_s=1.67;		% 2.4)  Local minimum factor smoothed_ps_to_smoothed_ps_minB_aposteriori_SNR_threshold
Bmin=1.66;
...
% Read input data
[y,Fs]=wavread(input_file_name,1);  % read size of input data, Fs and NBITS
N=wavread(input_file_name,'size');
N = N(1);
 
% Adjust parameters according to the actual sampling frequency
if Fs~=Fs_reference
    M=2^round(log2(Fs/Fs_reference*samples_per_frame_reference));
    Mo=overlapping_samples_per_frame_reference/samples_per_frame_reference*M;
    alpha_s=alpha_s_ref^(samples_per_frame_reference/M*Fs/Fs_reference);
    alpha_d=alpha_d_ref^(samples_per_frame_reference/M*Fs/Fs_reference);
    alpha_eta=alpha_eta_ref^(samples_per_frame_reference/M*Fs/Fs_reference);
    alpha_xi=alpha_xi_ref^(samples_per_frame_reference/M*Fs/Fs_reference);
else
    M=samples_per_frame_reference;
    Mo=overlapping_samples_per_frame_reference;
    alpha_s=alpha_s_ref;
    alpha_d=alpha_d_ref;
...
out=zeros(M,1);
b=hanning(2*frequency_smoothing_window_half_size+1);
...
finID = fopen([input_file_name '.wav'],'r');
for l=1:Nframes
    if l==1
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        aposteriori_SNR=Ya2./max(lambda_d,1e-10);
        smoothed_apriori_SNR=alpha_eta*eta_2term+(1-alpha_eta)*max(aposteriori_SNR-1,0);
        smoothed_apriori_SNR=max(smoothed_apriori_SNR,eta_min);
        v=aposteriori_SNR.*smoothed_apriori_SNR./(1+smoothed_apriori_SNR);

        % 2.1. smooth over frequency
        Sf=conv(b,Ya2);  % smooth over frequency
        Sf=Sf(frequency_smoothing_window_half_size+1:M21+frequency_smoothing_window_half_size);
        %         if l==1     % new version omlsa3
        if l==l_fnz     % new version omlsa3
            Sy=Ya2;
            S=Sf;
        ...
        conv_I=conv(b,I_f);
        conv_I=conv_I(frequency_smoothing_window_half_size+1:M21+frequency_smoothing_window_half_size);
        Sft=St;
        idx=find(conv_I);
        if ~isempty(idx)
            if frequency_smoothing_window_half_size
                conv_Y=conv(b,I_f.*Ya2);
                conv_Y=conv_Y(frequency_smoothing_window_half_size+1:M21+frequency_smoothing_window_half_size);
        ...
        phat(idx)=1./(1+qhat(idx)./(1-qhat(idx)).*(1+smoothed_apriori_SNR(idx)).*exp(-v(idx)));
        ...
        xi=alpha_xi*xi+(1-alpha_xi)*smoothed_apriori_SNR;
        ...
        PH1=zeros(M21,1);
        idx=find(q<0.9);
        PH1(idx)=1./(1+q(idx)./(1-q(idx)).*(1+smoothed_apriori_SNR(idx)).*exp(-v(idx)));

        % 7. Spectral Gain
        GH1=ones(M21,1);
        idx=find(v>5);
        GH1(idx)=smoothed_apriori_SNR(idx)./(1+smoothed_apriori_SNR(idx));
        idx=find(v<=5 & v>0);
        GH1(idx)=smoothed_apriori_SNR(idx)./(1+smoothed_apriori_SNR(idx)).*exp(0.5*expint(v(idx)));
        ...
        G=GH1.^PH1.*GH0.^(1-PH1);
        eta_2term=GH1.^2.*aposteriori_SNR;
    ...
    if l==1
        foutID = fopen([output_ '.wav'],'w');      % open output file and write first output samples
