% function audio_wiener_filter_with_wavelet_thresholding(input_file_name, output_file_name)
...
for frame_counter = 1:noisy_speech_number_of_frames
    ...
    %Estimate apriori SNR after cleaning noisy speech and dividing it by current estimated noise:
    current_final_apriori_linear_SNR_estimate = sum(current_clean_speech_power_spectrum_estimate) / sum(average_noise_power_spectrum);
    current_final_apriori_log_SNR_estimate(frame_counter) = 10*log10( current_final_apriori_linear_SNR_estimate + eps );

    %Decide on current lagrange multiplier used in the gain function:
    if current_final_apriori_log_SNR_estimate(frame_counter) >= 20
        lagrange_factor_current = over_substraction_factor_for_minimum_SNR;
    elseif current_final_apriori_log_SNR_estimate(frame_counter) < -5
        lagrange_factor_current = over_substraction_factor_for_maximum_SNR;
    else 
        lagrange_factor_current = over_substraction_factor_constant - current_final_apriori_log_SNR_estimate(frame_counter)*over_substraction_factor_constant_SNR_slope;
    end

    %Calculate the current gain function using the lagrange multiplier decided upon above:
    gain_function_current = current_clean_speech_power_spectrum_estimate ./ ( current_clean_speech_power_spectrum_estimate + lagrange_factor_current*average_noise_power_spectrum );
