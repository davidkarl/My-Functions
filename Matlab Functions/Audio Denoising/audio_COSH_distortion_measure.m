% function audio_COSH_distortion_measure(input_file_name,output_file_name,cost_function_p_power)
...
% input_file_name = 'poster_aftercleaning_170_audioPM final demodulated audio150-2100[Hz].wav';
...
input_signal = input_signal(round(Fs*1):end);
...
% bla = cumsum(final_enhanced_speech);
% filter_name_signal = 'hann';
% signal_filter_type = 'bandpass';
% signal_filter_length = 1000;
% signal_start_frequency = 200;
% signal_stop_frequency = 2200;
...
% bla = filter(signal_filter,bla); 
% bla = bla(signal_filter_length+1:end);
