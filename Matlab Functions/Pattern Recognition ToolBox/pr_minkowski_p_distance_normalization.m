%NORMM Fixed mapping for Minkowski-P distance normalisation
% 
%   B = NORMM(A,P)
%   B = A*NORMM([],P)
%   B = A*NORMM(P)
...
% (SIZE(A,2)=1), a second feature is added before the normalisation such 
% that A(:,2)=1-A(:,1).
% 
...
function w = pr_minkowski_p_distance_normalization(varargin)
  
	mapname = 'Normalisation Mapping';
  ...
  elseif mapping_task(argin,'training')			% Compute the mapping.
    [a,p] = deal(argin{:});
		...
		if (k == 1) & isdataset(a)	
			...
			a = [a 1-a];			% Add a second feature, handy for the normalisation.
			k = 2;
			prwarning(3,'SIZE(A,2)=1, so the second feature 1-A(:,1) is added.');
