...
function a = pr_generate_dataset_lithuanian_2_classes_2_dimensional(N,s)
		if nargin < 1, 
		prwarning(3,'Class cardinalities are not specified, assuming [50 50].');
		N = [50 50]; 
	end
	if nargin < 2, 
		prwarning(3,'Standard deviation for the data generation is not specified, assuming 1.');
		s = 1; 
	end
	if (length(N) == 1),
		N(2) = N(1);
