...
function handle = pr_plot_error_curve(varargin)
    ...
    xmax = 0;
        ...
        if noapperror
					hh = pr_plot_error_curve(e(j),linewidth,ss(j,:),'nolegend','errorbar','noapperror');
				else
					hh = pr_plot_error_curve(e(j),linewidth,char(ss(j,:),ss_app(j,:)),'nolegend','errorbar');
        end
			else
				if noapperror
					hh = pr_plot_error_curve(e(j),linewidth,ss(j,:),'nolegend','noapperror');
				else
					hh = pr_plot_error_curve(e(j),linewidth,char(ss(j,:),ss_app(j,:)),'nolegend');
				end
			end
			V = axis; 
      ymax = max(ymax,V(4));
