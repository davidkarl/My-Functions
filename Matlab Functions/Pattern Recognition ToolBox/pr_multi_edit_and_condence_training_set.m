...
function J = edicon (D,no_subsets,max_iter,no_tries)
    ...
    if (nargin < 4 | isempty(no_tries))
        prwarning(3,'number of tries for condensing not specified, assuming 10');
        no_tries = 10;
    end;
    if (nargin < 3 | isempty(max_iter))
        prwarning(3,'number of iterations for editing not specified, assuming 5');
        max_iter = 5;
    end;
    if (nargin < 2 | isempty(no_subsets))
        prwarning(3,'number of subsets for editing not specified, assuming 3');
        no_subsets = 5;
    end;
