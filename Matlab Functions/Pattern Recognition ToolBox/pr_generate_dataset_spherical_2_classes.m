...
function A = pr_generate_dataset_spherical_2_classes(N,k,ma,labtype)

    if nargin < 1
    N = [50 50];
    prwarning(3,'size of classes not specified, assuming [50 50]');
  end
  if nargin < 2
    k=2;
    prwarning(3,'dimension not specified, assuming 2');
  end
  if nargin < 3
    ma=0;
    prwarning(3,'mean not specified, assuming 0');
  end
  if nargin < 4
    labtype = 'crisp';
