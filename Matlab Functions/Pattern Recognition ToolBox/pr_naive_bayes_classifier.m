...
% Examples are PARZENM and GAUSSM. See also UDC.
%
% SEE ALSO (<a href="http://37steps.com/prtools">PRTools Guide</a>)
% DATASETS, MAPPINGS, PARZENM, GAUSSM, UDC, QDC, PARZENC, PARZENDC,
% REGOPTC, UDC
...
function w = pr_naive_bayes_classifier(varargin)
  ...
  elseif mapping_task(argin,'training')			% Train a mapping.
    ...
    if any(classsizes(a) < 2) % sufficiently large class sizes are needed
      prwarning(2,'training set size for some classes too small, fall back to UDC');
      w = udc(a);
      return
    end
    ...
    if ismapping(arg2) && isuntrained(arg2) % train given untrained mapping, e.g. parzen
      ...
      if isnan(N)             % optimise complexity parameter
        defs = {10};
        parmin_max = [2,50];
