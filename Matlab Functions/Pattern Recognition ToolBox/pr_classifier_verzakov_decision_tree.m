...
%
% DESCRIPTION
%    If (full grown) branches of (sub)tree do not improve classification  
%    error (misclassification cost) they are immediatley discarded. 
%    This may happen because we use regularised posteriors. As a result
%    the algorithm is more stable, trees are smaller, but splits on
%    the training set may be not perfect.
%
%    For efficiency the final classifier W consists of a feature selector
%    WSEL, selecting just the features used and the decision tree WDCT.
%    They can be retrieved by [WSEL,WDCT] = getdata(W). +WSEL returns the
%    feature set.
%
% REFERENCES
...
%    b = setfeatlab(b, getlabels(w));
    ...
    global FIRST; % workaround too early stopping
    FIRST = true;
    ...
    islabtype(a,'crisp');
    if numel(classuse(a,1)) < 2  % at least 1 objects per class, 2 classes
      prwarning(2,'training set too small: fall back to ONEC')
      w = onec(a);
      return
    end
    a = seldat(a,[],[],[],true);    % get labeled objects only
    ...
    L = unique(n(n>0)); % find features used in non-end nodes
    % create feature selector
    v = featsel(k,L);
    v = setsize(v,[k,numel(L)]);
    % fix the tree
    r = zeros(max(L)+1,1);
    r(L+1) = 1:numel(L);
    tree.fidx = r(n+1);
    ...
    % Store the results:
    w = v * prmapping('dtc', 'trained', tree, getlablist(a), numel(L), c);
    w = setname(w, ['DecTree' upper(crit)]);
    w = setcost(w, cost);
  end
...
function tree = makedt(a, feattype, usefeat, nfeat, c, nlab, weights, cost, crit, chisqstopval) 
  ...
  global FIRST; % work around too early stopping
  ...
  % regularisation by 'uniform' Bayesian priors;
  C0 = C;
  C = C + 1;
  p = C/sum(C);
  ...
  if NC ~= 1 % not a pure class dataset
    ...
    FIRST = false; % now we will have at least an elementary tree
...
function [critval, idx] = igr(feattype, varargin) 
  ...
  switch feattype
    case 0
      ...
      if ~isempty(R) 
        % solve 0/0  
        LL = L .* log2(L./(repmat(ML, [1 size(L, 2)])));
        RR = R .* log2(R./(repmat(MR, [1 size(R, 2)])));
        LLnan = find(isnan(LL));
        LL(LLnan) = zeros(1,numel(LLnan));
        RRnan = find(isnan(RR));
        RR(RRnan) = zeros(1,numel(RRnan));
        LL = reshape(LL,size(L));
        RR = reshape(RR,size(R));
        infolr = - (sum(LL, 2) + sum(RR, 2));
%         infolr = - ( ...
...
%         );
  ...
  if infogain > 0  || FIRST % here we make sure to get some result in the first call
