...
function h = parzenmlc(A,fid) %crisp version
	...
	% find zero-point of derivative to optimise h^2
	% stop if improvement is small, or h does not change significantly
	
	alf = 1;
	prwaitbar(100,'parzenml: Optimising smoothing parameter',m > 100)
	iter = 0;
  starttime = clock;
  runtime = 0;
...
function h = parzenmls(A,fid) %soft version
    ...
    runtime = 0;
		while abs(1e0-F2/F1) > 1e-4 & abs(1e0-h2/h1) > 1e-3 & abs(F2) > 1e-70
      if runtime > prt
        prwarning(2,'parameter optimisation for class %i stopped by PRTIME',j);
        break
      end
			iter = iter+1;
			prwaitbar(100,100-100*exp(-iter/10));
