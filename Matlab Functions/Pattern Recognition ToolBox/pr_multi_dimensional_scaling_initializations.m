%MDS_INIT Initialisation for MDS (variants of Sammon) mapping
% 
% 	Y = MDS_INIT (D,N,INIT)
% 
% INPUT
...
% 	INIT	Initialisation method (optional; default: 'randnp')
%
% OUTPUT
...
% The parameter INIT is a string standing for the initialisation method:
%
...
function [Y,w] = pr_multi_dimensional_scaling_initializations(D,n,initm)
		...
		switch initm		
			...
			case 'cs',
				if length(n) == 1 & m == mm,
					...
					error('The CS initialisation is not possible');
				end

			otherwise,   
				error('The possible initialisations are: randp, randnp, randv, maxv, cs or kl.');  
		end
	end
return;
