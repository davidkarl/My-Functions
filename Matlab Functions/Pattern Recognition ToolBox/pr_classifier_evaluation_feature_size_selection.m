...
function e = pr_classifier_evaluation_feature_size_selection(a,classf,featsizes,learnsize,n,Tset,testfun)
  ...
  a = setprior(a,getprior(a));
	...
	e.apperror = zeros(r,length(featsizes));
	e.appstd   = zeros(r,length(featsizes));
	e.xvalues  = featsizes;
	e.n = n;
	datname = getname(a);
	if ~isempty(datname)
	...
	for q = 1:r
		...
		et = zeros(n,length(featsizes)); % test error
		ea = zeros(n,length(featsizes)); % apparent error
        ...
        w = b(:,1:f)*classf{q};        % train the classifier
				et(i,j) = T(:,1:f)*w*testfun;  % test error
				ea(i,j) = b(:,1:f)*w*testfun;  % apparent error
			end
			prwaitbar(0)
		end
		prwaitbar(0)
		e.error(q,:) = mean(et,1);
		e.apperror(q,:) = mean(ea,1);
		if n == 1
			e.std(q,:) = zeros(1,size(e.std,2));
			e.appstd(q,:) = zeros(1,size(e.appstd,2));
		else
			e.std(q,:) = std(et)/sqrt(n);
			e.appstd(q,:) = std(ea)/sqrt(n);
		end
	end
	prwaitbar(0)
