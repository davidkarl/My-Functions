...
function [W,J,NU] = nulibsvc(varargin)
  ...
  if mapping_task(argin,'definition')
    ...
    pp = prrmpath('libsvm','svmtrain');     % check / correct path
    finishup1 = onCleanup(@() addpath(pp)); % restore afterwards
		u = svmtrain(nlab,K,opt);
    
		if isempty(u)
			prwarning(2,'optimisation failed, LKC is used instead')
      if isequal(kernel,0)
        W = lkc(setdata(a,K(:,2:end)),0);
        J = [1:m]';
      else
        W = lkc(a,kernel);
      end
    ...
    pp = prrmpath('libsvm','svmpredict');     % check / correct path
    finishup2 = onCleanup(@() addpath(pp));  % restore afterwards
