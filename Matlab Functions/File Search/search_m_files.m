function Outfiles=search_m_files(baseDir,searchExpression,flag_recursive,flag_return_full_path)
...
relevant_files = dir(fullfile(baseDir,strcat('*',searchExpression,'*.m')));
...
if flag_recursive==1
    for k=1:length(folder_names)
       ...
       OutfilesTemp = search_m_files(new_fullfile_name{1},strcat('*',searchExpression,'*.m'),flag_recursive,flag_return_full_path);
