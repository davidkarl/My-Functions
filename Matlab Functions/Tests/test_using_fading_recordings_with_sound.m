...
file_name='turbulence_1900pm_high_power_new';
...
% bla = fread(fid_turbulence2,samples_per_frame*50,'double');
bla = fseek(fid_turbulence2,samples_per_frame*4*30,-1);
...
%     filtered_carrier2 = step(carrier_filter_object,current_frame_turbulence2);
    ...
    analytic_signal1 = (abs(analytic_signal1).^2.*exp(1i*angle(analytic_signal1)) + abs(analytic_signal2).^2.*exp(1i*angle(analytic_signal2)) ) ./ (abs(analytic_signal1).^2 + abs(analytic_signal2).^2);
%     figure;
%     subplot(1,2,1);
