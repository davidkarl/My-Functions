...
% signal_filter_fft = signal_filter.freqz(FFT_size_signal_filter);
...
while frame_counter<number_of_frames_to_read
    ...
    if flag_show_large_frame_tic_toc==1
        disp('(1) carrier filter:');
        toc
    end
    ...
    tic
    current_t_vec = current_t_vec + (frame_counter-1)*(samples_per_frame_demodulation/Fs);
    if flag_show_large_frame_tic_toc==1
        disp('(3) t_vec:');
        toc
    end
    ...
    if flag_show_large_frame_tic_toc==1
        disp('(4) multiply by exp(-2*pi*i*Fc*t_vec):');
        toc
    end
    ...
    if flag_show_large_frame_tic_toc==1
        disp('(5) angle(bla(2:end)).*conj(angle(bla(1:end-1))):');
        toc
    end
    ...
    if flag_show_large_frame_tic_toc==1
        disp('(7) find indices below click threshold:');
        toc
    end   
    ...
    %(6). get sub-framed & buffered signal matrix and window them:
    tic
    phase_signal_matrix = buffer(phase_signal_after_mask,samples_per_frame,overlap_samples_per_frame);
    if flag_show_large_frame_tic_toc==1
        disp('(9) buffer phase signal matrix:');
        toc
    end
    tic
    phase_signal_matrix = bsxfun(@times,phase_signal_matrix,hanning_window);
    if flag_show_large_frame_tic_toc==1
        disp('(10) multiply by hanning window:');
        toc
    end
    
    %(7). calculate ffts and spectrums with number of elements larger then original size for later overlap-add:
    tic
    phase_signal_fft_matrix = fft(phase_signal_matrix,FFT_size_signal_filter);
    if flag_show_large_frame_tic_toc==1
        disp('(11) FFT buffered phase signal matrix:');
        toc
    end
    tic
    phase_signal_spectrum_matrix = abs(phase_signal_fft_matrix).^2;
    if flag_show_large_frame_tic_toc==1
        disp('(12) abs(buffered_fft).^2:');
        toc
        disp('***************');
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    while sub_frame_counter < global_to_sub_frame_size_ratio*2
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(1) assign current_sub_frame stuff:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(2) update indices using mod:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(3) update ffts_history etc with current stuff:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(4) get current search windows to variable:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(5) sort current search windows:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(6) calculate current search windows mean:');
            toc
        end
        ...
        if current_max_sample < number_of_initial_samples_containing_only_noise
            ...
            elseif noise_profile_flag==2
                ...
                initial_noise_data_spectrum_running_max_of_min = max(initial_noise_data_spectrum_matrix_running_local_min,[],2);
        ...
        if current_max_sample > number_of_initial_samples_containing_only_noise
            ...
            if flag_show_sub_frame_tic_toc==1
                disp('(7) find indices above noise gate:');
                toc
            end
            ...
            if flag_show_sub_frame_tic_toc==1
                disp('(8) decay attack and release:');
                toc
            end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(9) assign current final gain and fft into seperate variables:');
            toc
        end
        ...
        if flag_show_sub_frame_tic_toc==1
            disp('(10) multiply current fft by gain function and signal filter:');
            toc
        end
        tic
