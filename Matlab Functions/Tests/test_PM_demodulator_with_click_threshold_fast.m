...
while counter<number_of_samples_to_read
    ...
    analytic_signal_mask = zeros(samples_per_frame,1);
    analytic_signal_mask(indices_to_disregard_click) = 1;
    ...
    flag_use_despiking_or_just_my_mask_or_nothing = 3;
...
%     filtered_phase = irlssmooth(phase_signal_difference,10,0);
%     filtered_phase = powersmooth(filtered_phase,2,1);
%     filtered_phase = smooth(filtered_phase,20,'lowess');
%     filtered_phase = smoothn(filtered_phase,25);
%     filtered_phase = hampel(filtered_phase,10);
%     filtered_phase = medfilt1(filtered_phase,10);
%     filtered_phase = sgolayfilt(filtered_phase,3,5);

    
    %(8). Turn to PM if wanted
    flag_PM_or_FM = 1; 
    if flag_PM_or_FM==1;
        final_signal = cumsum(filtered_phase)+last_cumsum;
