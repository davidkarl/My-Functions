function y = parallelogram_upsampling(mat_in, type, flag_polyphase_component)
% PUP   Parallelogram Upsampling
%
% 	y = pup(x, type, [phase])
%
% Input:
%	x:	input image
%	type:	one of {1, 2, 3, 4} for selecting sampling matrices:
%			P1 = [2, 0; 1, 1]
%			P2 = [2, 0; -1, 1]
%			P3 = [1, 1; 0, 2]
%			P4 = [1, -1; 0, 2]
%	phase:	[optional] 0 or 1 to specify the phase of the input image as
%		zero- or one-polyphase	component, (default is 0)
%
% Output:
%	y:	parallelogram upsampled image
%
% Note:
%	These sampling matrices appear in the directional filterbank:
%		P1 = R1 * Q1
%		P2 = R2 * Q2
%		P3 = R3 * Q2
%		P4 = R4 * Q1
%	where R's are resampling matrices and Q's are quincunx matrices
%
% See also:	PPDEC

if ~exist('phase', 'var')
    flag_polyphase_component = 0;
end

% Parallelogram polyphase decomposition by simplifying sampling matrices
% using the Smith decomposition of the quincunx matrices
%
% Note that R1 * R2 = R3 * R4 = I so for example,
% upsample by R1 is the same with down sample by R2.
% Also the order of upsampling operations is in the reserved order
% with the one of matrix multiplication.

[m, n] = size(mat_in);

up = 1;
down = 2;
left = 3;
right = 4;
switch type
    case 1	% P1 = R1 * Q1 = D1 * R3
        y = zeros(2*m, n);
        
        if flag_polyphase_component == 0
            y(1:2:end, :) = shift_matrix_columnwise_or_rowwise(mat_in, right);
        else
            y(2:2:end, [2:end, 1]) = shift_matrix_columnwise_or_rowwise(mat_in, right);
        end
        
    case 2	% P2 = R2 * Q2 = D1 * R4
        y = zeros(2*m, n);
        
        if flag_polyphase_component == 0
            y(1:2:end, :) = shift_matrix_columnwise_or_rowwise(mat_in, left);
        else
            y(2:2:end, :) = shift_matrix_columnwise_or_rowwise(mat_in, left);
        end
        
    case 3	% P3 = R3 * Q2 = D2 * R1
        y = zeros(m, 2*n);
        
        if flag_polyphase_component == 0
            y(:, 1:2:end) = shift_matrix_columnwise_or_rowwise(mat_in, down);
        else
            y([2:end, 1], 2:2:end) = shift_matrix_columnwise_or_rowwise(mat_in, down);
        end
        
    case 4	% P4 = R4 * Q1 = D2 * R2
        y = zeros(m, 2*n);
        
        if flag_polyphase_component == 0
            y(:, 1:2:end) = shift_matrix_columnwise_or_rowwise(mat_in, up);
        else
            y(:, 2:2:end) = shift_matrix_columnwise_or_rowwise(mat_in, up);
        end
        
    otherwise
        error('Invalid argument type');
end
