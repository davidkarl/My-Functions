function [rotation_angles_cell_array , coherency_cell_array] = ...
                             riesz_monogenic_analysis_of_riesz_coefficients(riesz_wavelet_matrices_cell_array, ...
...
if flag_restrict_angle_value == 1, %compute sign of the direction thanks to the gradient of the wavelet coefficients
    %Initialize another riesz transform object with order 0 (just wavelet transform):
    wavelet_transform_object2 = riesz_transform_object(size(mat_in), 0, number_of_scales, 1);
    
    %compute multiscale wavelet decomposition:
    mat_in_wavelet_gradient_angle = multiscale_riesz_analysis(mat_in, wavelet_transform_object2);
