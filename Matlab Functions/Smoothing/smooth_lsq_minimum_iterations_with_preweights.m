function [lsq_solution_signal_final] = smooth_lsq_minimum_iterations_with_preweights(input_signal,raw_signal_weights,derivative_signal_weight_factor,differentiation_order)
...
derivative_signal_weight_coefficient_normalizers=[4.000 3.416 3.404 3.411 3.417 3.4 3.4 3.4 3.4]; %don't really know where these come from
...
while flag_lsq_equation_is_ill_conditioned && current_differentiation_order>=1
    ...
    %I don't understand this....but doesn't really matter...just a weight factor:
    normalized_derivative_signal_weight_factor = (derivative_signal_weight_factor...
                /derivative_signal_weight_coefficient_normalizers(current_differentiation_order)) ^ current_differentiation_order;
    ...
    flag_which_derivative_signal_weights_to_use = 3;
    if flag_which_derivative_signal_weights_to_use == 1
        derivative_signal_weights = raw_signal_weights(1:input_signal_length-current_differentiation_order);
    elseif flag_which_derivative_signal_weights_to_use == 2
        derivative_signal_weights = ones(size(raw_signal_weights(1:input_signal_length-current_differentiation_order)));
    else
        derivative_signal_weights = 1 - raw_signal_weights(1:input_signal_length-current_differentiation_order);
    end
...
%     v = [raw_signal_weights.*input_signal; zeros(input_signal_length-current_differentiation_order,1)];
    v = [raw_signal_weights.*input_signal; derivative_signal_weights.^1 .* differentiated_signal];
